...
import 'package:lbs/templates/BOOKSTUDENT.dart';
...
class _View_Student_DetailsState extends State<View_Student_Details> {
  ...
  Future<List<BOOKSTUDENT>> getBooksStudentData(String registerno) async {
    ...
    if (response.statusCode == 200)
      {
        final List bookstudent = jsonDecode(response.body);
  ...
  Future studentRenewDate(String isbnno1) async {
    var url = "http://localhost/lbs_php/renewBookStudentData.php";
    final response = await http.post(Uri.parse(url),
    body: {
  ...
  Future studentReturnBook(String isbnno2) async {
    var url = "http://localhost/lbs_php/returnBookStudentData.php";
    final response = await http.post(Uri.parse(url),
    body: {
      "registerno" : registerno.text,
      "isbnno" : isbnno2,
