...
import 'package:lbs/Book/Provide_Book_Staff.dart';
import 'package:lbs/Book/Provide_Book_Student.dart';
import 'package:lbs/Staff/Add_Staff.dart';
import 'package:lbs/Librarian/Update_Librarian.dart';
import 'package:lbs/Student/Add_Student.dart';
import 'package:lbs/Book/Add_Book.dart';
import 'package:lbs/Login_Page.dart';
import 'package:lbs/Book/View_Book.dart';
import 'package:lbs/Staff/View_Staff.dart';
import 'package:lbs/Student/View_Student.dart';
import 'package:flutter_styled_toast/flutter_styled_toast.dart';
import 'package:lbs/templates/LIBRARIAN.dart';
import 'package:http/http.dart' as http;

class Home_Page extends StatefulWidget {
  final String username ;
  final String password ;
  const Home_Page({Key? key, required this.username, required this.password}) : super(key: key);
...
class _Home_PageState extends State<Home_Page> {
  ...
  {
    var url = "http://localhost/lbs_php/getLibrarianData.php";
    final response = await http.post(Uri.parse(url),
        body: {"username": widget.username, "password": widget.password,});
    if (response.statusCode == 200) {
      final List librarian = jsonDecode(response.body);
      return librarian.map((json) => LIBRARIAN.fromJson(json)).toList();
    } else {
      throw Exception();
    }
  }
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: ()
                {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => Update_Librarian(
                    firstname: librarian[0].firstname,
                    lastname: librarian[0].lastname,
                    email: librarian[0].email,
                    librarianid: librarian[0].librarianid,
            ...
            child: ListView(
              ...
              children: <Widget>[
                UserAccountsDrawerHeader(
                  ...
                  accountName: Text("${firstname.text} ${lastname.text}", style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold),),
                  accountEmail: Text(email.text, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold),),
                  currentAccountPicture: CircleAvatar(
                    backgroundColor: Colors.pink,
                    child: Text(firstname.text.toString().substring(0,1),
