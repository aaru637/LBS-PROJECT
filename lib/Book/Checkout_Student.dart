...
class _Checkout_StudentState extends State<Checkout_Student> {
  ...
  Future checkOutBookStudentData(
      String booktitle1,
      String edition1,
      String publishedyear1,
      String isbnno1,
      String publication1,
      String registerno1) async {
    var url = "http://localhost/lbs_php/checkOutBookStudentData.php";
    ...
    print(res);
    if (res == "true") {
      customStudentToast(booktitle1 + " CheckedOut SuccessFully...", context);
      Navigator.pop(context);
    } else {
      customErrorToast('You are already CheckedOut...', context);
      clearText();
    }
  }

  Future bookCountUpdate(String isbnno3) async {
    var url = "http://localhost/lbs_php/bookCountUpdate.php";
    final response = await http.post(Uri.parse(url), body: {
      "isbnno": isbnno3,
    });
  ...
  Future staffCountUpdate(String register) async {
    var url = "http://localhost/lbs_php/studentCountUpdate.php";
    final response = await http.post(Uri.parse(url), body: {
      "registerno": register,
    });
    ...
    if (data == 'true') {
      ...
      customStudentToast('Chceked Out Successfully', context);
    } else if (data == 'count exceeded') {
      customErrorToast(
          'You are already borrow limited books. So You can\'t borrow...',
          context);
    } else {
