...
import 'dart:convert';
import 'package:lbs/templates/BOOKSTAFF.dart';
...
class _View_Staff_DetailsState extends State<View_Staff_Details> {
  ...
  Future<List<BOOKSTAFF>> getBooksStudentData(String registerno) async {
    var url = "http://localhost/lbs_php/getStudentBookData.php";
    ...
    {
      final List bookstaffs = jsonDecode(response.body);
  ...
  Future staffRenewDate(String isbnno1) async {
    var url = "http://localhost/lbs_php/renewBookStaffData.php";
    final response = await http.post(Uri.parse(url),
        body: {
  ...
  Future staffReturnBook(String isbnno2) async {
    var url = "http://localhost/lbs_php/returnBookStaffData.php";
    final response = await http.post(Uri.parse(url),
        body: {
          "registerno" : registerno.text,
          "isbnno" : isbnno2,
  ...
  Future<List<BOOKSTAFF>> getStaffBookData() async {
    var url = "http://localhost/lbs_php/getStaffBookData.php";
    final response = await http.post(Uri.parse(url),
        body: {'registerno': registerno.text.toUpperCase()});
  ...
  Future init() async {
    final bookstaffs = await getStaffBookData();
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) => View_Staff_Book_Details(
                        ...
                        checkdate: bookstaff.checkdate,
                        renewdate: bookstaff.renewdate,
                      )));
            },
            trailing: Row(
