...
import 'package:lbs/Staff/Update_Staff.dart';
...
class _View_StaffState extends State<View_Staff> {
  ...
  Future<List<STAFF>> getStaffData(String query) async {
    final url = Uri.parse("http://localhost/lbs_php/getStaffData.php");
    final response = await http.get(url);

    if (response.statusCode == 200) {
      final List staffs = jsonDecode(response.body);
      return staffs.map((json) => STAFF.fromJson(json)).where((staff) {
        final register = staff.registerno.toUpperCase();
        final name = staff.firstname.toUpperCase();
  ...
  Future deleteStaffData(String registerno1) async {
    var url = "http://localhost/lbs_php/deleteStaffData.php";
    var response =
